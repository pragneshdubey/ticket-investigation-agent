...
from backend.models.v1_agent import V1TriageRequest

ROOT_DIR = Path(__file__).resolve().parents[2]
EVAL_INPUT_PATH = ROOT_DIR / "backend" / "data" / "evaluation" / "eval_tickets.json"
EVAL_OUTPUT_PATH = ROOT_DIR / "evaluation" / "v1" / "v1_results.json"
...
def main() -> None:
    ...
    print(f"Starting V1 Agent Evaluation on {len(tickets)} tickets...")
    ...
    for idx, ticket in enumerate(tickets, start=1):
        ...
        response = agent.run(V1TriageRequest(ticket_id=ticket_id, text=ticket_text))
    ...
    payload = {
        "evaluator": "V1 Triage Agent Evaluator",
        "models": {
