...
def _get_next_run_id_unlocked(runs: List[Dict[str, Any]]) -> str:
    """Internal helper to calculate next run ID from existing runs list."""
...
def get_next_run_id() -> str:
    ...
    """
    with _run_lock:
        return _get_next_run_id_unlocked(get_runtime_runs())
...
def persist_agent_run(
    ...
    Complete READ-MODIFY-WRITE critical section protected by _run_lock.
    """
    with _run_lock:
        ...
        run_id = existing_run["run_id"] if existing_run else _get_next_run_id_unlocked(runs)
